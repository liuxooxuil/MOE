...
abstract contract MOERewards is MOEBinding {
    function calculateRewardRates() public view returns (
        uint256 staticRate, uint256 dynamicRate, uint256 nftRate, uint256 projectRate
    ) {
    ...
    function calculateStaticReward(address user) public view returns (uint256) {
        ...
        if (!_user.isBuy || !_user.staticDrawStatus || block.timestamp < _user.staticDrawAt) return 0;
